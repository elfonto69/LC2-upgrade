...
class ManageOrderEngine implements ManageOrderEngineBlueprint
{
    ...
    public function processDeleteOrders($inputData, $orderId)
    {
        ...
        if (__isEmpty($order)) {
            return __engineReaction(18, null, __('Order not exist.'));
        }

        // Check if order deleted
        if (!$this->manageOrderRepository->deleteOrder($inputData, $order)) {
            return __engineReaction(2, null, __('Please enter correct password.'));
        }

        return __engineReaction(1, null, __('Order deleted Successfully.'));
    }
    ...
    public function processDeleteSandboxOrder($orderId)
    {
        ...
        if (in_array($order->payment_method, $paymentMethod) !== true) {
            return __engineReaction(18, null, __('This is not a sandbox order.'));
        }

        if ($this->manageOrderRepository->deleteSandboxOrder($order)) {
            return __engineReaction(1, null, __('Sandbox order deleted Successfully.'));
        }

        return __engineReaction(2, null, __('Sandbox order not deleted.'));
    }
}
