...
/*
 * Core Request - 0.2.21 - 04 FEB 2017
 *
    ...
    protected function normalizeEncryptedInput()
    {
        ...
        foreach ($allInputs as $key => $value) {
            ...
            if (in_array($key, $this->unsecuredFields) === false
                ...
                // decrypt input value
                $descriptedValue = YesSecurity::decryptRSA($value);

                $allInputs[$key] = $descriptedValue;
                // if cannot be decrypt may long concated string are there
                if (!$descriptedValue) {
                    // make parts of it via specified string chars
                    $splitedValues = explode('__==__', $value);
                    $splitedString = '';

                    foreach ($splitedValues as $splitedValue) {
                        $splitedString .= YesSecurity::decryptRSA($splitedValue);
