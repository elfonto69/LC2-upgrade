...
class ProductEngine implements ProductEngineBlueprint
{
    ...
    public function priceFilter($productPrices, $input)
    {
        ...
        if (isset($input['min_price']) and isset($input['max_price'])) {
            $showFilterPrice = priceFormat($input['min_price'], false, true).
                                __(' to ')
                                .priceFormat($input['max_price'], false, true);
        }
    ...
    public function getProductForDetails($product)
    {
        ...
        if (!__isEmpty($product->option)) {
            foreach ($product->option as $optionKey => $option) {
                ...
                $getPrice['total'] = priceFormat($product->price + array_sum($addonPrice), false, true);
                $getPrice['base_price'] = priceFormat($product->price, false, true);
            }
        }
        ...
        $product->newTotalPrice = priceFormat($product->price, true, true);

        if (!empty($addonPrice)) {
            $totalAddonPrice = array_sum($addonPrice);
            $product->newTotalPrice = priceFormat($product->price + $totalAddonPrice, true, true);
    ...
    public function getQuickViewDetailsData($productID, $categoryID)
    {
        ...
        $detailURL = route('product.details', (!empty($categoryID))
                    ...
                    : ['productID' => $productId, 'productName' => slugIt($productName)]);
        ...
        $productDetails['oldPrice' ] = ($productDetails->old_price) ? priceFormat($productDetails->old_price, true, true) : '';
