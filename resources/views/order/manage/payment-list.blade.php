...
<script type="text/template" id="orderPaymentActionTemplate">
    ...
    <% if(__tData.isTestOrder) { %>
        ...
        <a title="<?= __('Delete Sandbox Order') ?>" href="" ng-click="paymentListCtrl.deleteSandBoxOrder('<%- __tData.order_payment_id %>', '<%- __tData.order_uid %>', 1)" class="btn btn-danger btn-xs"><i class="fa fa-trash" aria-hidden="true"></i> <?= __('Delete') ?></a>
        <!-- Delete Sandbox Order button -->  
    <% } else { %>
        <!-- Delete Sandbox Order button -->
        <a title="<?= __('Delete Sandbox Order') ?>" href="" ng-click="paymentListCtrl.deleteSandBoxOrder('<%- __tData.order_payment_id %>', '<%- __tData.order_uid %>', 2)" class="btn btn-danger btn-xs"><i class="fa fa-trash" aria-hidden="true"></i> <?= __('Delete') ?></a>
        <!-- Delete Sandbox Order button -->  
    <% } %>
</script>