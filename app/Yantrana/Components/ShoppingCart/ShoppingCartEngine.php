...
class ShoppingCartEngine implements ShoppingCartEngineBlueprint
{
    ...
    public function updateCartString()
    {
        ...
        if (empty($takeCartItems)) {
            return $this->generateCartString([
                'totalItems' => 0,
                'itemsInCart' => 'item',
                'cartTotal' => priceFormat(0, true, true),
            ]);
        }
    ...
    public function getCartBtn()
    {
        ...
        return __engineReaction(1, [
                ...
                'cartString' => $this->generateCartString([
                        ...
                        'cartTotal' => priceFormat($cartStringData['cartTotal'], true, true),
                    ]),
            ]);
    }
    ...
    */
    ...
    public function getProductCartData($productID, $inputs)
    {
        ...
        $data['totalPrice'] = priceFormat(($totalAddonPrice + $product->price), true, true);
        $data['formatedTotalPrice'] = priceFormat($totalAddonPrice + handleCurrencyAmount($product->price), true, true);
        $data['base_price'] = priceFormat(handleCurrencyAmount($product->price), false, true); // base price of product
    ...
    public function processUpdateQty($input, $itemID)
    {
        ...
        $oldCartContent = [
            ...
            'new_subTotal' => priceFormat($subTotalPrice, true, true),
        ];
