...
class ManageOrderController extends BaseController
{
    ...
    public function index($status, $userID)
    {
        ...
        $requireColumns = [
            ...
            'creation_date' => function ($key) use ($status) {
                return ($status == 1)
                    ? ((isset($key['created_at'])) ? formatStoreDateTime($key['created_at']) : '')
                    : ((isset($key['uodated_at'])) ? formatStoreDateTime($key['uodated_at']) : '');
            },
     ...
     * @return json object
     *---------------------------------------------------------------- */
    public function deleteSandboxOrder($orderId)
    {
        $processReaction = $this->manageOrderEngine
